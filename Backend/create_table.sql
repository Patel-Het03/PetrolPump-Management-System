...
INSERT INTO `PetrolPump` (`Registration_No`, `Petrolpump_Name`,'Latitude','Longitude',`Company_Name`, `Opening_Year`, `State`, `City`) VALUES
('HPC805103', 'Sumaraj Petroleum',3,4, 'Hindustan Petroleum',2016,'Bihar','Hisua'),
('BP110054', 'Rajinder Service Station',8,10, 'Bharat Petroleum',2012,'Delhi','CENTRAL DELHI'),
('IOC560008', 'Madhu Enterprises',6,8, 'Indian Oil Corporation',2008,'Karnataka','Banglore'),
...
CREATE TABLE IF NOT EXISTS `Sales`(
   ...
   `Petrolpump_No`varchar(10) DEFAULT NULL,
   `Time_Interval` Time DEFAULT 00:00:00
   PRIMARY KEY(`Sales_No`,`Date`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

INSERT INTO `Sales` (`Sales_No`, `Date`, `Nozzel_No`, `Starting_Reading`, `Ending_Reading`, `Total_Sales`, `Sales_Amount`, `Petrolpump_No`,`Time_Interval`) VALUES
('FGHGE32','2022-11-20', 1, 45687,49782 , 17584.45, 106.52,'HPC805103','12:30:46'),
('MVBER67','2022-11-20', 2, 48325, 53842, 4253.45, 205.5,'OIL380013','2:20:15'),
('IUOSF98','2019-08-19', 2, 12757, 23454, 1254.71, 89.45,'HPC805103','17:56:04'),
('GDZJD24','2019-08-19', 1, 62725,68725 , 5466.45, 125.85,'OIL380013','20:50:56'),
('QWRGH87','2022-11-22', 3, 12758, 19758, 7854.65, 425.25,'HPC805103','6:34:25');
...
-- Change the data types and table/column names as needed
...
BEGIN
    ...
    -- Retrieve the fuel price for the specified tanker ID
    SET RATE = (SELECT FUEL_PRICE FROM TANKER WHERE TANKER_ID = TID);
    
    -- Retrieve the fuel amount for the specified tanker ID
    SET VOL = (SELECT FUEL_AMOUNT FROM TANKER WHERE TANKER_ID = TID);
    
    -- Calculate the total bill by multiplying the rate and volume
    SET BILL = RATE * VOL;
